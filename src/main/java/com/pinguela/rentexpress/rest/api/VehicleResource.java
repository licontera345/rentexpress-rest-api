...
public class VehicleResource {
    ...
    public Response notificarFinMantenimiento(FinalizarMantenimiento dto) {
        if (dto == null || dto.getMatricula() == null) {
            return Response.status(Status.BAD_REQUEST).entity("La matrícula es obligatoria").build();
        }

        try {
            // Este método en el service guarda el mensaje en la columna 'description'
            vehicleService.finishMaintenance(dto.getMatricula(), dto.getDescripcion());
            RedisCache.deleteByPrefix("vehicles:");
            return Response.ok().entity("{\"mensaje\":\"Aviso de mantenimiento registrado correctamente\"}").build();
