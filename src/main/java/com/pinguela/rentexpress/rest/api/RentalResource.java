...
public class RentalResource {
    ...
    public Response findByCriteria(
        ...
        if (securityContext != null && securityContext.getUserPrincipal() != null) {
            try {
                ...
                return Response.status(Status.BAD_REQUEST).entity("Invalid user context").build();
            }
        } else {
            criteria.setUserId(userId);
        }
    ...
    public Response completeRental(@PathParam("id") Integer id, RentalDTO rentalData) {
        ...
        try {
            ...
            if (completed == null) {
                return Response.status(Status.NOT_FOUND).entity("Rental not found after completion").build();
            }
            RedisCache.deleteByPrefix("rentals:");
